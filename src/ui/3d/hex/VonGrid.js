...
export class VonGrid {
  constructor(config = {}) {
    /*  ______________________________________________
      GRID INTERFACE:
    */
    this.type = constants.HEX;
    this.size = 5; // only used for generated maps
    this.cellSize =
      typeof config.cellSize === 'undefined' ? 10 : config.cellSize;
    this.cells = {};
    this.numCells = 0;
    ...
    */
    ...
    this._cellLength = constants.SQRT3 * 0.5 * this._cellWidth;
    this._hashDelimeter = '.';
  ...
  pixelToCell(pos) {
    ...
    const q = pos.x * (VonGrid.TWO_THIRDS / this.cellSize);
    const r = (-pos.x / 3 + constants.SQRT3 / 3 * pos.z) / this.cellSize;
  ...
  getRandomCell() {
    let i = 0;
    const x = tools.randomInt(0, this.numCells);
    let c;
    for (c in this.cells) {
      if (i === x) {
        return this.cells[c];
  ...
  generateTiles(config) {
    ...
    const settings = tools.merge(defaultSettings, config);
  ...
  generateTilePoly(material) {
    ...
    mesh.rotateOnAxis(this._vec3, constants.PI / 2);
    return mesh;
  }
    ...
    for (x = -size; x < size + 1; x++) {
      for (y = -size; y < size + 1; y++) {
        ...
        if (Math.abs(x) <= size && Math.abs(y) <= size && Math.abs(z) <= size) {
          ...
          line.rotation.x = 90 * constants.DEG_TO_RAD;
          overlayObj.add(line);
        }
      }
    }
  ...
  */
  ...
  /*_pixelToAxial(pos) {
    ...
    r = ((-pos.x / 3) + (constants.SQRT3/3) * pos.y) / this.cellSize;
    this._cel.set(q, r, -q-r);
    return this._cubeRound(this._cel);
  }*/
  ...
  /*_hexToPixel(h) {
    ...
    y = this.cellSize * constants.SQRT3 * (h.y + (h.x * 0.5));
    return {x: x, y: y};
  }*/
